...
export function EntrySheet({
  ...
  function save() {
    ...
    if (
      ...
      setError("Enter a number from 0 to 1,000,000.");
      return;
    }
    onSave(meetsTarget(rule, number) ? "met" : "not-met", number);
  }
  return (
    <Sheet
      title={habit.name}
      ...
      onClose={onClose}
    >
      <div className={`entry-intro ${habit.color}`}>
      ...
      {rule.kind === "number" ? (
        ...
        >
          <label className="field">
            ...
            />
            {error && (
              <span id="number-error" className="field-error">
                {error}
              </span>
            )}
          </label>
