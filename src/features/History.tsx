    ...
    <>
      ...
      {habits.length ? (
        ...
        <div className="empty-state">
          <h2>Your story starts with one habit.</h2>
          <p>A small check-in today becomes a bigger picture over time.</p>
      ...
      <section className="weekly-review">
        <div className="review-heading">
          <div>
            ...
            <h2>Last week, at a glance.</h2>
          </div>
          <span>
            {formatDate(previousWeek, { month: "short", day: "numeric" })} –{" "}
            {formatDate(addDays(previousWeek, 6), {
              month: "short",
              day: "numeric",
            })}
          </span>
        ...
        {summary.total ? (
          <>
            <div className="review-totals">
              ...
              <div>
                <strong>{summary.unlogged}</strong>
                <p>not logged</p>
              </div>
            </div>
            <div className="review-rows">
              {summary.rows.map((row) => (
                <div
                  key={row.habit.id}
                  className={`review-row ${row.habit.color}`}
                >
                  <div>
                    <span className="color-dot" />
                    <strong>{row.habit.name}</strong>
                    <small>
                      {row.current} current · {row.best} best streak
                    </small>
                  </div>
                  <div className="review-bar">
                    <i
                      style={{
                        width: `${Math.round((row.met / row.total) * 100)}%`,
                      }}
                    />
                  </div>
                  <span>{Math.round((row.met / row.total) * 100)}%</span>
                  <button
                    className="icon-button"
                    aria-label={`Adjust ${row.habit.name}`}
                    onClick={() => onEdit(row.habit)}
                  >
          ...
          <p className="help-text">
            Your first weekly review will appear here after you’ve started
            tracking. There’s no need to fill in the past.
          </p>
        )}
      </section>
    </>
